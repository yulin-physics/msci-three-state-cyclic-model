...
def logbin(data, scale = 1.3, zeros = False):
    ...
    count = np.bincount(data) #np.bincount finding the frequency of each number in the input array
    tot = np.sum(count)
    smax = np.max(data)
    if scale > 1:
        jmax = np.ceil(np.log(smax)/np.log(scale)) #np.ceil finding the upper closest integer
        if zeros:
            binedges = scale ** np.arange(jmax + 1)
            binedges[0] = 0
        else:
            binedges = scale ** np.arange(1,jmax + 1) #binedges will return the bins with edges in ln
            # count = count[1:]
        binedges = np.unique(binedges.astype('uint64')) #unit64 is unsigned long np.unique return the sorted unique values in the array
