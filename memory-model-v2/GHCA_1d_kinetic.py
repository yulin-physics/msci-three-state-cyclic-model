...
R = 10 #Refractory period/time step delay from R to Q
T = 1000 #Number of time steps to run for

class GHCA:
    def __init__(self,size=L, refrac=R, tot_t=T):
        self.L=size
        self.R=refrac 
        self.T=tot_t
    ...
    def run(self):
        ...
        for i in range(self.T):
            ...
            #random number of excited sites
            limit = np.random.randint(5)
            start=0
            stop=self.L
        
            self.rand_site = [np.random.randint(start, stop) for iter in range(limit)]
            extra_array = np.zeros(self.L,dtype=np.bool)
            extra_array[self.rand_site] = True
            #print(extra_array)
            ...
            self.exc_array += extra_array #Add excitations
