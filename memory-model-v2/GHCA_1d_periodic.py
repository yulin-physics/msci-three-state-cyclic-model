...
lattice = GHCA()
lattice.run()
#data = pd.read_csv('p00015_test_periodic.csv', header=None)
#lattice.plot_leaderDistribution()
#lattice.plot_leaderDrift(data[1].values)
# =============================================================================
# data = pd.read_csv('2048.csv', header=None)
# lattice.plot_leaderDrift(data[0], data[1])
# =============================================================================

# =============================================================================
# data = pd.read_csv('2048_log.csv', header=None)
# 
# plt.figure(3)
# ax = plt.gca()
...
#     k, m = np.polyfit(np.log(x), np.log(y), 1)
#     return np.exp(m) * xnew**(k), k
# ys = powerfit(data.iloc[:,0], data.iloc[:,1], data[0])
# ax.plot(data[0], ys[0], color='k')   
# ax.scatter(data[0], data[1], color='k')
# ax.set_yscale('log')
# ax.set_xscale('log')
# ax.set_xlabel('Δt')
# ax.set_ylabel('Δn')
# ax.text(5,84,'H=(%s)'%(ys[1]))
# print(ys[1])
# =============================================================================
...
# =============================================================================
# data = pd.read_csv('4096_dn.csv', header=None)
# 
# prep_y=list(data[1])
# prep_x=list(data[0])
# i=0
# while i < 10:    
#     ind_1 = np.argmax(prep_y)
#     prep_y.remove(max(prep_y))
#     prep_x.remove(prep_x[ind_1])
#     i += 1
# 
# j=0
# while j < 1200:    
#     ind_1 = np.argmin(prep_y)
#     prep_y.remove(min(prep_y))
#     prep_x.remove(prep_x[ind_1])
#     j += 1
# 
# plt.figure(3)
...
#     k, m = np.polyfit(np.log(x), np.log(y), 1)
#     return np.exp(m) * xnew**(k), k
# ys = powerfit(prep_x, prep_y, data[0])
# ax.plot(data[0], ys[0], color='k')   
# ax.scatter(data[0], data[1], color='k')
# ax.set_yscale('log')
# ax.set_xscale('log')
